...
Give every file you write a sorted `related:` frontmatter list and a closing `## Related` markdown-link section: each `api/<m>.md` → `modules/<m>` and `symbol-map`; `symbol-map.md` → `architecture`; `errors.md` → `patterns` and the modules whose error types it documents. Links are relative from the file's own directory — from `api/foo.md` write `../modules/foo.md` and `../symbol-map.md`. Verify each resolves before you return.
